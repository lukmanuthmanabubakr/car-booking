...
const Hero = () => {
    ...
    return (
        <div className='h-screen flex flex-col items-center justify-center gap-14 bg-light text-center'>
            ...
            <form className='flex flex-col md:flex-row items-center justify-between p-6 rounded-full w-full max-w-80 md:max-w-200 bg-white shadow[0px_8px_20px_rgba(0,0,0,0.1)]'>
                <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
                    <div className='flex flex-col items-start gap-2'>
                        <select required value={pickupLocation} onChange={(e) => setPickupLocation(e.target.value)}>
                            <option value="">Pickup Location</option>
                            {cityList.map((city, index) => <option key={city} value={city}>{city}</option>)}
                        </select>
